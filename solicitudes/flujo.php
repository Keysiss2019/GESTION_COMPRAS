...
if (strcasecmp($estado, "nueva") === 0) {
    ...
    echo "<br>";
    echo "<a href='../cotizaciones/add_cotizacion.php?id=$solicitudId' class='yellow-btn'><i class='fas fa-book'></i>COTIZAR</a>";
    echo "</div>";
} else {
    echo "<div style='display: flex; flex-direction: column; align-items: center;'>";
    echo "<i class='far fa-check-circle check-circle'></i>";
    echo "<br>";
    echo "<span class='disabled-btn'><i class='fas fa-book'></i>COTIZAR</span>";
    echo "</div>";
}
...
if (strcasecmp($estado, "pendiente") === 0) {
    ...
    echo "<i class='far fa-check-circle check-circle'></i>";
    echo "<br>";
    echo "<a href='../cotizaciones/view_solicitud.php?id=$solicitudId' class='green-btn'><i class='fas fa-book-open'></i>APROBAR</a>";
    echo "</div>";
} else {
    echo "<div style='display: flex; flex-direction: column; align-items: center;'>";
    echo "<i class='far fa-check-circle check-circle'></i>";
    echo "<br>";
    echo "<span class='disabled-btn'><i class='fas fa-book-open'></i> APROBAR</span>";
    echo "</div>";
}
...
if (strcasecmp($estado, "proceso") === 0) {
    ...
    echo "<i class='far fa-check-circle check-circle'></i>";
    echo "<br>";
    echo "<a href='../cotizaciones/detalle_solicitud.php?id=$solicitudId' class='orange-btn'><i class='fas fa-file-invoice'></i> GENERAR COMPRAR</a>";
    echo "</div>";
} else {
    echo "<div style='display: flex; flex-direction: column; align-items: center;'>";
    echo "<i class='far fa-check-circle check-circle'></i>";
    echo "<br>";
    echo "<span class='disabled-btn'><i class='fas fa-file-invoice'></i> GENERAR COMPRAR</span>";
    echo "</div>";
}
...
echo "<div style='margin-left: 500px;'>"; // Contenedor principal del botón de retroceder
echo "<select onchange='redirectToPage(this)' class='blue-btn-dropdown retroceder-btn'>";
echo "<option disabled selected>--- Atras ---</option>";

// Opción para retroceder a la página de agregar cotización
echo "<option value='../solicitudes/editar_cotizacion.php?id=$solicitudId'>AGREGAR COTIZACIÓN</option>";

// Opción para retroceder a la página de ver solicitud si el estado es "nueva"
if (strcasecmp($estado, "nueva") !== 0) {
    echo "<option value='../cotizaciones/view_solicitud.php?id=$solicitudId'>APROBAR COTIZACIÓN</option>";
}

// Opción para retroceder a la página de detalle de solicitud si el estado es "pendiente" o "proceso"
if (strcasecmp($estado, "nueva") !== 0 && strcasecmp($estado, "pendiente") !== 0) {
