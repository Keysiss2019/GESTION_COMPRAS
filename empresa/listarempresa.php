...
<head>
    ...
    <style>
        ...
        #empresasTable tr:nth-child(even) {
            background-color: #f2f2f2;
        }
...
<body>
    <div class="content">
        ...
        if (isset($_GET['busqueda'])) {
            ...
            $stmt = $conn->prepare("SELECT * FROM tbl_empresa WHERE id_empresa = :busqueda OR nombre_empresa LIKE :busqueda");
            $stmt->bindValue(':busqueda', $busqueda);
            $stmt->execute();
            $empresas = $stmt->fetchAll(PDO::FETCH_ASSOC);
        } else {
            // Si no se ha realizado una búsqueda, muestra todas las empresas
            $stmt = $conn->query("SELECT * FROM tbl_empresa");
            $empresas = $stmt->fetchAll(PDO::FETCH_ASSOC);
        }
        ?>
        ...
        <h1 style="display: flex; align-items: center;">
            <span class="fas fa-question"></span> Empresas
            <a href="crear_empresa.php" class="print-button" style="margin-left: 10px; padding: 5px 10px; border-radius: 5px; background-color: #3b2ad3; color: #fff; font-size: 14px;"><i class="fas fa-plus"></i> </a>
        </h1>

        <!-- Tu tabla HTML -->
        <table id="empresasTable" class="display">
            <thead>
                <tr>
                    <th>ID</th>
            ...
            <tbody>
                <?php foreach ($empresas as $empresa) { ?>
                    <tr>
                        ...
                        <td><?php echo $empresa['tel_empresa']; ?></td>
                        ...
                        <td><?php echo $empresa['fecha_modificacion']; ?></td>
                        <td class="d-flex align-items-center">
                            <a href="editar_empresa.php?id=<?php echo $empresa['id_empresa']; ?>" class="edit-link"><i class="fas fa-edit"></i></a>
                            <a href="eliminar_empresa.php?id=<?php echo $empresa['id_empresa']; ?>" class="delete-link"><i class="fas fa-trash-alt"></i></a>
                        </td>
                    </tr>
                <?php } ?>
            </tbody>
